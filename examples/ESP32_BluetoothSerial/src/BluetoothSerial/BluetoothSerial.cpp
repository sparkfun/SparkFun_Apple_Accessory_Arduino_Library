...
{
    if ((xEventGroupWaitBits(_spp_event_group, SPP_CONGESTED, pdFALSE, pdTRUE, SPP_CONGESTED_TIMEOUT) &
         SPP_CONGESTED) != 0)
    {
        if (!_spp_client)
        {
...
static void esp_spp_cb(esp_spp_cb_event_t event, esp_spp_cb_param_t *param)
{
    ...
    case ESP_SPP_DISCOVERY_COMP_EVT: // Enum 8 - When SDP discovery complete
        ...
        if (param->disc_comp.status == ESP_SPP_SUCCESS)
        {
            ...
            {
                log_d("ESP_SPP_DISCOVERY_COMP_EVT: spp [%d] channel: %d service name:%s", i, param->disc_comp.scn[i],
                      param->disc_comp.service_name[0]);
            }
            if (_doConnect)
            {
                if (param->disc_comp.scn_num > 0)
                {
                    ...
                    log_i("ESP_SPP_DISCOVERY_COMP_EVT: spp connect to remote %s channel %d",
                          bda2str(_peer_bd_addr, bda_str, sizeof(bda_str)), param->disc_comp.scn[0]);
#endif
...
static void esp_bt_gap_cb(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_DISC_RES_EVT: { // Enum 0 - Device discovery result event
        ...
        for (int i = 0; i < param->disc_res.num_prop; i++)
        {
            ...
            case ESP_BT_GAP_DEV_PROP_RSSI: // Enum 3 - Received Signal strength Indication, value type is int8_t,
    ...
    case ESP_BT_GAP_RMT_SRVCS_EVT: // Enum 2 - Get remote services event
        log_i("ESP_BT_GAP_RMT_SRVCS_EVT: status = %d, num_uuids = %d", param->rmt_srvcs.stat,
              param->rmt_srvcs.num_uuids);
        break;
...
{
    TickType_t xTicksToWait = timeout / portTICK_PERIOD_MS;
    return (xEventGroupWaitBits(_spp_event_group, BT_DISCOVERY_COMPLETED, pdFALSE, pdTRUE, xTicksToWait) &
            BT_DISCOVERY_COMPLETED) != 0;
}
...
static bool waitForSDPRecord(int timeout)
{
    ...
    TickType_t xTicksToWait = timeout / portTICK_PERIOD_MS;
    return (xEventGroupWaitBits(_bt_event_group, BT_SDP_COMPLETED, pdFALSE, pdTRUE, xTicksToWait) & BT_SDP_COMPLETED) !=
           0;
}
...
void BluetoothSerial::enableSSP()
{
    ...
    {
        log_i("Attempted to enable SSP for already initialized driver. Restart to take effect with end() followed by "
              "begin()");
        return;
    }
    _enableSSP = true;
...
// When Input is true and Output is false User will be required to input the passkey to the ESP32 device to
// authenticate.
...
bool BluetoothSerial::disconnect()
{
    if (_spp_client)
    {
        ...
        if (esp_spp_disconnect(_spp_client) == ESP_OK)
        {
            ...
            return (xEventGroupWaitBits(_spp_event_group, SPP_DISCONNECTED, pdFALSE, pdTRUE, xTicksToWait) &
                    SPP_DISCONNECTED) != 0;
        }
    }
    return false;
